...
# --- Funções Auxiliares de Entrada (Sem alterações) ---
...
def gerar_problema_transporte(n_fabricas, n_cds):
    print("\n--- Modo de Geração Aleatória ---")
    
    num_variaveis = n_fabricas * n_cds
    c_transporte = np.random.randint(1, 11, size=num_variaveis)
    
    demandas = np.random.randint(500, 1001, size=n_cds)
    ofertas = np.random.randint(400, 1101, size=n_fabricas)

    print(f"Dimensões: {n_fabricas} Fábricas -> {n_cds} CDs")
    print(f"Oferta Total: {np.sum(ofertas)} | Demanda Total: {np.sum(demandas)}")
    print("AVISO: Problema aleatório provavelmente não-balanceado. O otimizador lidará com isso.")
    
    return c_transporte, ofertas, demandas, n_fabricas, n_cds

# --- Funções de Otimização (Sem alterações) ---
...
def plotar_solucao(solucao, n_fabricas, n_cds, n_fab_orig, n_cds_orig, cd_ficticio, fabrica_ficticia):
    """
    Gera um gráfico de barras. (Sem alterações)
    """
    ...
    for i in range(n_fabricas):
        ...
        ax.bar_label(barras, fmt='%.0f', padding=3, fontsize=9, 
                     labels=[f'{v:.0f}' if v > 0.01 else '' for v in dados_fabrica])
...
def plotar_grafo_rede(solucao, n_fab_orig, n_cds_orig, ofertas_orig, demandas_orig, custos_orig, cd_ficticio, fabrica_ficticia):
    """
    Gera um gráfico de rede (grafos) bipartido.
    *** VERSÃO CORRIGIDA (Bug das cores) ***
    """
    ...
    for i, nome in enumerate(nomes_fabricas):
        ...
        cores_fabricas.append("skyblue") # Cor Correta: Azul
    
    # 3. Processar CDs Originais
    for j, nome in enumerate(nomes_cds):
        custom_labels[nome] = f"{nome}\n(Demanda: {demandas_orig[j]:.0f})"
        cores_cds.append("lightgreen") # Cor Correta: Verde
    ...
    if fabrica_ficticia:
        ...
        falta_total = np.sum(solucao[-1, :n_cds_orig]) 
        custom_labels[nome_ficticio] = f"{nome_ficticio}\n(Total: {falta_total:.0f})"
        cores_fabricas.append("lightcoral") # Cor Correta: Vermelho (para a fábrica)

    if cd_ficticio:
        nome_ficticio = "SOBRA (CD Fictício)"
        nomes_cds.append(nome_ficticio) # Adiciona à lista de nomes
        sobra_total = np.sum(solucao[:n_fab_orig, -1])
        custom_labels[nome_ficticio] = f"{nome_ficticio}\n(Total: {sobra_total:.0f})"
        cores_cds.append("silver") # Cor Correta: Cinza (para o CD)

    # 5. Adicionar nós ao grafo (na ordem correta)
    # Esta ordem define como as cores serão aplicadas
    G.add_nodes_from(nomes_fabricas, bipartite=0)
    G.add_nodes_from(nomes_cds, bipartite=1)
    
    # 6. Criar lista de cores final NA ORDEM CORRETA
    # Primeiro as cores das fábricas, depois as cores dos CDs
    node_colors = cores_fabricas + cores_cds 
    
    # 7. Adicionar Arestas (Setas) e Rótulos (sem mudança aqui)
    edge_labels = {}
        ...
        for j in range(n_cds_total):
            ...
            if quantidade > 0.01: 
                ...
                if is_real_fabrica and is_real_cd:
                    ...
                    label_custo = "Custo: R$ 0.00"
                
                edge_labels[(origem, destino)] = f"{label_qtd}\n{label_custo}"

    # 8. Desenhar o Grafo (agora com a lista 'node_colors' correta)
    ...
    nx.draw(G, 
            ...
            node_color=node_colors, # <-- Esta lista agora está correta
            node_size=5500,
            font_size=9,
            font_weight='bold', 
            arrows=True,
...
if choice == '1':
    ...
    c_custos_orig, b_ofertas_orig, b_demandas_orig, n_fab_orig, n_cds_orig = gerar_problema_transporte(n_fab_rand, n_cds_rand)
...
# --- Otimização (Agora com problema balanceado) ---
...
bounds_variaveis = [(0, None) for _ in range(n_vars)]
